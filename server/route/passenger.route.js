...
async function addNewPassenger(req, res) {
  try {
    ...
    // generate id
    let id = 'P'+ (Math.floor(Math.random()*100));
    id = newPassenger?.seatNumber ? id+newPassenger.seatNumber : newPassenger?.flightId;
    id = id + (Math.floor(Math.random()*10789));

    newPassenger['id'] = id;

    await passengers.insertMany(newPassenger);
    const passenger = await passengers.find({id: newPassenger.id});
    res.send(passenger[0]);
    // let message = '';
    // if (result.length >= 1) {
    //   res.statusCode = 201;
...
passengerRoute.get('', async (req, res) => {
  try {
    ...
    if(flightId){
      const passengersData = await passengers.find({ flightId: flightId });
      res.send(passengersData);
    }else{
      throw new Error('flightId is no mentioned');
    }
    // let passengersData = [];
    // let message = '';
    // let code = 5000;
...
passengerRoute.get('/:id', async (req, res) => {
  try {
    ...
    const passenger = await passengers.find({ id: passengerId });
    res.send(passenger[0]);
    // let passengerData = [];
    // let resObj = {
    //   passenger: {},
...
passengerRoute.patch('/:id', async (req, res) => {
  try {
    ...
    await passengers.updateOne({ id: passengerId }, payload);
    const passenger = await passengers.find({id: passengerId});
    res.send(passenger[0]);
    // let message = '';
